...
execute context x = case x of
   ...
   SFor stmi exp stmu stme -> execute context (SBlock [stmi, SWhile exp (SBlock[stme,stmu]) ])
   SIf  exp stm -> if ( (eval context exp) /= 0) 
                      then execute context stm
                      else context
   SIfE exp stmt stme -> if ( (eval context exp) /= 0) 
